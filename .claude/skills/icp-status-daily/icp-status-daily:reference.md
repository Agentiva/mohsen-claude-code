...
**Update 02.09.2026:** Stand jetzt 124 Zeilen gesamt, davon 42 `Gekündigt` →
82 Zeilen im Lauf. Normale Verschiebung durch Kündigungen seit dem 31.08., kein
Fehler.
...
⚠️ **Update 02.09.2026 – veraltet:** Die Live-Daten in Notion verwenden inzwischen
durchgehend `Kampgane aktivieren` statt `Kampgane erstellen` (verifiziert über
alle 21 betroffenen Zeilen im Lauf vom 02.09., keine einzige Zeile trug noch
`Kampgane erstellen`). Zwischen dem 31.08. und dem 02.09. wurde der Anzeigetext
der Option offenbar in Notion umbenannt (Options-ID vermutlich unverändert,
nur das Label). Bis das hier korrigiert ist: **`Kampgane aktivieren` schreiben,
nicht `Kampgane erstellen`.** Vor dem nächsten Lauf mit einer kurzen
Live-Stichprobe (`SELECT DISTINCT "ICP-Status" ...`) gegenprüfen, falls sich das
nochmal ändert.
...
`Leads hinzufügen`, `Keine Emails raus gesendet`, `Kampgane aktivieren`
(bis auf Weiteres statt `Kampgane erstellen`, siehe oben),
...
mcp__Amplifa-Sales-Hub__onb_list { "full": true, "limit": 100, "offset": 0 }

// Nur im Onboarding-Gate
mcp__Amplifa-Sales-Hub__onb_list_domains { "onboarding_id": "<uuid>" }
mcp__Amplifa-Sales-Hub__onb_list_senders { "onboarding_id": "<uuid>" }

// Schreiben
mcp__Amplifa-Sales-Hub__onb_update { "onboarding_id": "<uuid>", "patch": { "icp_status": "Live" } }
mcp__Amplifa-Sales-Hub__onb_sync_notion { "onboarding_id": "<uuid>" }
```

(Hinweis 02.09.2026: Die tatsächlichen MCP-Tool-Namen verwenden Bindestriche,
`mcp__Amplifa-Sales-Hub__...`, nicht Unterstriche wie oben ursprünglich notiert –
rein kosmetisch, betrifft nur die Schreibweise in dieser Referenz.)

⚠️ **Neu, 02.09.2026 – `onb_sync_notion` aktuell defekt:** Bei jedem Aufruf in
diesem Lauf (9 von 9 Statusänderungen) schlug `onb_sync_notion` serverseitig fehl:
„ICP-Status is expected to be multi_select. Organisation erstellt in Paltform is
not a property that exists." Der Fehler ist unabhängig vom eigentlichen
ICP-Status-Feld – die Sync-Automation referenziert offenbar ein nicht (mehr)
existierendes Notion-Property namens „Organisation erstellt in Paltform"
(Tippfehler „Paltform" statt „Plattform"). Der „Verifizierter Schreibpfad"-Test
unten von 28.08.2026 lief noch fehlerfrei – die Regression ist also zwischen dem
28.08. und dem 02.09. entstanden. **Workaround bis zum Fix:** `onb_update` normal
ausführen, `onb_sync_notion` versuchen; schlägt es fehl, den ICP-Status-Wert
direkt per `notion-update-page` in Notion setzen (Schritt c vorziehen bzw. auch
bei Einzelstatus ausführen). Muss technisch behoben werden.
...
Update 02.09.2026: Zusätzlich zu prüfen, ob die Notion-Zeile im Feld
„Amplifa Plattform" bereits einen Deeplink zu einer Org-ID trägt (siehe unten,
Fall REMIRA Austria GmbH) – dieser Link hat Vorrang vor Namens-Fuzzy-Matching.
...
⚠️ Stand 02.09.2026 nicht mehr reproduzierbar – `onb_sync_notion` schlägt jetzt
serverseitig fehl, siehe Warnung weiter oben unter "Tool-Calls".
